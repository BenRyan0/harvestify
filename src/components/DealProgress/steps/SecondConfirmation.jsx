...
import { paymentAdd ,messageClear,cancel_transaction_due_to_dispute_by_trader,resend_deposit_proof} from '../../../store/reducers/transactionReducer';
...
const [traderReason, setTraderReason] = useState("");
 const handTraderInput = (e) => setTraderReason(e.target.value);
     ...
     {currentTransaction[0]?.dispute[0]?.status === "In-Dispute" ? (
        <div className="w-full">
          <div className="">
            ...
            <div className="flex flex-row md:flex-col h-fit gap-3">
        ...
        currentTransaction[0]?.dispute[0]?.status === "Resolved" && currentTransaction[0]?.dispute[0]?.status === ""? (
          <div className="w-full">
  ...
  {openModal ? (
          <div
            className="absolute inset-0  flex justify-center items-center bg-slate-800 pb-[100px] bg-opacity-30 transition-opacity duration-900 ease-in-out z-[999]"
            onClick={() => setOpenModal(false)}
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className=" shadow-md bg-[#F1F5F9] h-[200px] md-lg:h-[250px] ml-[280px] md-lg:ml-0 w-8/12 md-lg:w-11/12 relative transform transition-all duration-900 ease-in-out scale-100 opacity-100"
            >
              <div className="p-4 text-white">
                <div className="font-bold text-primaryDark">PLEASE SELECT A CHOICE OF ACTION</div>
                <div className="w-full flex justify-center items-center">
                  <div className=" grid grid-cols-3 gap-8 w-9/12 font-bold mt-10 sm:flex sm:flex-col sm:gap-2">
                    <button
                    onClick={submitCancellation}
                    className="bg-primary py-2 px-4 rounded uppercase text-white font-semibold transition duration-200 ease-in-out hover:bg-primaryDark focus:outline-none focus:ring-2 focus:ring-primaryDark"
                  >
                    Cancel
                  </button>

                    <button onClick={() => setOpenResendModal(true)} className="bg-primary py-2 px-4 rounded uppercase text-white font-semibold transition duration-200 ease-in-out hover:bg-primaryDark focus:outline-none focus:ring-2 focus:ring-primaryDark">
                      Resend
                    </button>

                    <button className="bg-primary py-2 px-4 rounded uppercase text-white font-semibold transition duration-200 ease-in-out hover:bg-primaryDark focus:outline-none focus:ring-2 focus:ring-primaryDark">
                      Insist
                    </button>
  ...
  {openResendModal ? (
          <div
            className="absolute inset-0   flex justify-center items-center bg-blue-800 pb-[100px] bg-opacity-30 transition-opacity duration-900 ease-in-out z-[999]"
