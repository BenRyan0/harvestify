...
const Details = () => {
  ...
  const { listing, relatedListings, moreListings, totalShippingFee } =
  ...
  const handleAddCard = (id) => {
    if (userInfo) {
      dispatch(
        add_to_card({
          // Ensure `add_card` here refers to the action creator, not the function name
          userId: userInfo.id,
          quantity: 1,
          listingId: id,
        })
        ...
        <div>
            ...
            <div className="absolute left-0 top-0 w-full h-full bg-[#03872D] bg-opacity-40">
              <div className="w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto">
                <div className="flex flex-col justify-center gap-1 items-center h-full w-full text-center text-white">
                  <img
                    className="w-[220px]"
                    src={`http://localhost:3001/images/Harvestify_logo_full.png`}
                    alt=""
                  />
                </div>
              </div>
          ...
          <section>
            <div className="w-[85%] bg-slate-100 md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto py-5 px-3 pb-16">
              <div className="grid grid-cols-5 md-lg:grid-cols-1 gap-8">
                <div className="relative col-span-2">
                  <div className="">
                    <img
                      className="h-[300px] w-full rounded"
                      src={image ? image : listing.images?.[0]}
                      alt=""
                    />
                  </div>
                      ...
                      >
                        {listing.images.map((img, i) => {
                          return (
                            <div
                              key={i}
                              className="px-1 h-[120px]"
                              onClick={() => setImage(img)}
                            >
                              <img
                                className="cursor-pointer h-full w-full rounded-sm"
                                src={img}
                                alt=""
                              />
                            </div>
                  ...
                  <div className="flex flex-col gap-2 justify-start items-start absolute w-[65px] h-[200px] font-semibold text-xs left-2 top-2 transition-all duration-700 z-50">
                    <ul className="flex transition-all duration-700 left-1 top-6 justify-center items-center absolute w-[100px] opacity-0 group-hover:left-[66px] group-hover:w-[90px] group-hover:opacity-100">
                ...
                <div className="flex flex-col gap-5 col-span-3">
                  ...
                  <div className="flex justify-start items-center">
                    <div className="flex gap-1">
                      <img
                        className="h-[50px] rounded-full border-primaryDark border-2"
                         ...
                         <div className="flex justify-center items-center">
                          ...
                          <h2 className="text-lg text-primary pr-1">
                            {formatNumber(
                              listing.totalPrice -
                                Math.floor(
                                  (listing.totalPrice * listing.discount) / 100
                                )
                            )}
                          </h2>
                          <p className="line-through text-slate-500">
                            {formatNumber(listing.totalPrice)}
                          </p>
                          <p className="flex items-center text-xs text-gray-500 bg-primary/50 mx-1 px-1 rounded-md">
                            {" "}
                            -{listing.discount}%
                  ...
                  <div className="text-slate-600 text-sm w-8/12">
                    ...
                    <div className="flex justify-start items-center flex-row gap-1">
                      ...
                      listing.additionalFeatures.length > 0 ? (
                        listing.additionalFeatures.map((feature, i) => (
                          <div
                            key={i}
                            className="text-white px-2 py-1 bg-primary/80 rounded-md font-semibold text-sm flex gap-1 items-center"
                          >
                            {feature}
                            <span>
                              <FaCircleCheck />
                  ...
                  <div className="flex py-5 gap-5">
                    ...
                    <div className="flex flex-col gap-5">
                      ...
                      <ul className="flex justify-start items-center gap-3">
                        <li>
                          <a
                            className="group w-[38px] h-[38px] hover:bg-accent hover:text-white text-slate-200 flex justify-center items-center bg-primaryDark rounded-full"
                            href="/"
                          >
                  ...
                  <div className="flex md-lg:flex-col py-5 gap-3">
                    {stock ? (
                      <button
                        onClick={() => redirect_(listing)}
                        className="h-[50px] group flex justify-center items-center gap-2 font-bold bg-primaryDark px-5 py-2 rounded-md text-slate-100 hover:shadow-md"
                      >
                        Take Deal
                        <FaHandshake
                          size={25}
                    ...
                    <Link
                      ...
                      className="h-[50px] block group flex justify-center items-center gap-2 font-bold bg-primaryDark px-5 py-2 rounded-md text-slate-100 hover:shadow-md"
                    >
                      Chat Seller
                      <IoChatboxEllipses
                        size={25}
                  ...
                  <div className="pr-4 md-lg:pr-0">
                    <div className="grid grid-cols-2 font-semibold gap-1">
                      <button
                        ...
                        className={`py-3 px-5 transition-colors duration-200 hover:bg-primaryDark hover:text-white ${
                          state === "reviews"
                        ...
                        return (
                          ...
                          >
                            <div className="relative h-[270px]">
                              ...
                              />
                              {p.discount ? (
                                <div className="flex justify-center items-center absolute text-white py-1 px-2 gap-1 rounded-md bg-primaryDark/80 font-semibold text-xs left-2 top-2">
                                  {" "}
                                  -{p.discount}% <IoTicketSharp />
                                </div>
                              ) : (
                                <div className=""></div>
                              )}
                              ...
                              {p.discount ? (
                                ...
                                <div className="flex flex-col">
                                  <div className="flex flex-row gap-2 items-center">
                                    <h2 className="">
                                      Price: <span>&#8369;</span>20/kl
                                    </h2>
                                    <div className="border-l-4 h-[70%] border-primaryDark"></div>
                                    <h2 className="">2tn</h2>
                                  </div>
                                  <h2 className="">
                                    Sum Total: <span>&#8369;</span> 40000{" "}
                                  </h2>
                                </div>
                              )}
                            </div>
