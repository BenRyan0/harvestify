      ...
      <div className="loading-screen h-screen bg-green-800/90 flex justify-center items-center">
        <div className="relative inline-flex">
          <div className="rounded-full bg-green-400 h-[50px] w-[50px] inline-block mr-2">
            <img src="/logo512.png" className="w-full h-full" alt="" />
          </div>
          <div className="absolute animate-ping rounded-full bg-green-400 h-[50px] w-[50px] mr-2"></div>
        </div>
        <div className="absolute bottom-20 bg-white p-2 rounded-md max-w-[350px]">
          <h2 className="font-bold text-[11px]">Disclaimer: </h2>
          <p className="text-center pt-2 text-[9px]">
            Our backend runs on Render’s free tier, which goes to sleep when
            inactive. This can cause the first request to load slowly. We added
            a loading screen to show while the server wakes up. After that,
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route
          path="/clusters/cluster-details/:clusterId"
          element={<ClusterListings />}
        />
        ...
        <Route path="/dashboard" element={<ProtectUser />}>
          <Route path="" element={<Dashboard />}>
            ...
            <Route
              path="cancellation/:transactionId"
              element={<CancellationPAge />}
            />
            <Route
              path="cancellation-deal/:dealId"
              element={<CancellationDeal />}
            />
          </Route>
        </Route>
      </Routes>
